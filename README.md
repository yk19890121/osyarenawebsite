...
- **GIMMICKS** (`/gimmicks/`) — Webでよく使う演出・ギミック160種を11カテゴリーに分類した実験カタログ
...
120種の旧カタログ（#08と#09は1デモに統合のため実質119エントリ）に、新規41種を追加した
計160種を、11カテゴリーに再編しています。
...
- ギミック: プレビューでライブに動くのは14種類(text-mask-reveal, split-character, fade-up,
  ...
  残り146種類（160種 − ライブ14種）は「GIMMICK カタログ指定」機能でオブジェクトごとに
